...
import { Button, Center, FlatList, Input, NativeBaseProvider } from "native-base";
  ...
  async function addEditions() {
    try {
      EDITIONS.forEach(async (edition) => {
        console.log(edition.title, "\t", edition.authors[0].name, "\t", edition.publishers[0]);
        const e = {
          title: edition.title,
          authors: [{name: edition.authors[0].name, ol_key: edition.authors[0].key}],
